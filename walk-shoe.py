...
# get the peaks, using a threshold of thrld_1 and a distance between peaks of 55
indexes_1 = peakutils.peak.indexes(numpy.array(accel_1),thres=thrld_1/max(accel_1), min_dist=55)

# get the peaks, using a threshold of thrld_2 and a distance between peaks of 63
indexes_2 = peakutils.peak.indexes(numpy.array(accel_2),thres=thrld_2/max(accel_2), min_dist=63)

# get the peaks, using a threshold of thrld_3 and a distance between peaks of 40
indexes_3 = peakutils.peak.indexes(numpy.array(accel_3),thres=thrld_3/max(accel_3), min_dist=40)

sum = len((indexes_1)) + len((indexes_2)) + len((indexes_3)) 

print('Steps are: %d' % len((indexes_1)))
#print('Steps in Second Accelerometer reading are: %d' % len((indexes_2)))
#print('Steps in Third Accelerometer reading are: %d' % len((indexes_3)))
